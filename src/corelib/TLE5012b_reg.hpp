 ...
 * \name        TLE5012b_reg.hpp - Library for Arduino for the TLE5012B angle sensor.
...
public:
	...
	typedef struct stat_t //!< Status register 0x00
	{
		...
		bool fetch_SRST(uint16_t reg)
		{
			SRST = (reg & 0x1);
			return (SRST);
		} //!< status reset
		bool fetch_SWD(uint16_t reg)
		{
			SWD = (reg & 0x2) >> 1;
			return (SWD);
		} //!< status watch dog
		bool fetch_SVR(uint16_t reg)
		{
			SVR = (reg & 0x4) >> 2;
			return (SVR);
		} //!< status voltage regulator
		bool fetch_SFUSE(uint16_t reg)
		{
			SFUSE = (reg & 0x8) >> 3;
			return (SFUSE);
		} //!< status fuses
		bool fetch_SDSPU(uint16_t reg)
		{
			SDSPU = (reg & 0x10) >> 4;
			return (SDSPU);
		} //!< status digital signal processing unit
		bool fetch_SXYOL(uint16_t reg)
		{
			SXYOL = (reg & 0x20) >> 5;
			return (SXYOL);
		} //!< status overflow
		bool fetch_SOV(uint16_t reg)
		{
			SOV = (reg & 0x40) >> 6;
			return (SOV);
		} //!< status X/Y data out limit
		bool fetch_SMAGOL(uint16_t reg)
		{
			SMAGOL = (reg & 0x80) >> 7;
			return (SMAGOL);
		} //!< status magnitude out limit
		bool fetch_Reserved1(uint16_t reg)
		{
			Reserved1 = (reg & 0x100) >> 8;
			return (Reserved1);
		} //!< reserved
		bool fetch_SADCT(uint16_t reg)
		{
			SADCT = (reg & 0x200) >> 9;
			return (SADCT);
		} //!< status ADC test
		bool fetch_SROM(uint16_t reg)
		{
			SROM = (reg & 0x400) >> 10;
			return (SROM);
		} //!< status ROM
		bool fetch_NOGMRXY(uint16_t reg)
		{
			NOGMRXY = (reg & 0x800) >> 11;
			return (NOGMRXY);
		} //!< no valid GMR XY Values
		bool fetch_NOGMRA(uint16_t reg)
		{
			NOGMRA = (reg & 0x1000) >> 12;
			return (NOGMRA);
	...
	typedef struct acstat_t //!< Activation Status register offset 0x01
	{
		...
		bool fetch_ASRST(uint16_t reg)
		{
			ASRST = (reg & 0x1);
			return (ASRST);
		}
		bool fetch_ASWD(uint16_t reg)
		{
			ASWD = (reg & 0x2) >> 1;
			return (ASWD);
		}
		bool fetch_ASVR(uint16_t reg)
		{
			ASVR = (reg & 0x4) >> 2;
			return (ASVR);
		}
		bool fetch_ASFUSE(uint16_t reg)
		{
			ASFUSE = (reg & 0x8) >> 3;
			return (ASFUSE);
		}
		bool fetch_ASDSPU(uint16_t reg)
		{
			ASDSPU = (reg & 0x10) >> 4;
			return (ASDSPU);
		}
		bool fetch_ASOV(uint16_t reg)
		{
			ASOV = (reg & 0x20) >> 5;
			return (ASOV);
		}
		bool fetch_ASVECXY(uint16_t reg)
		{
			ASVECXY = (reg & 0x40) >> 6;
			return (ASVECXY);
		};
		bool fetch_ASVEGMAG(uint16_t reg)
		{
			ASVEGMAG = (reg & 0x80) >> 7;
			return (ASVEGMAG);
		}
		bool fetch_Reserved2(uint16_t reg)
		{
			Reserved2 = (reg & 0x100) >> 8;
			return (Reserved2);
		}
		bool fetch_ASADCT(uint16_t reg)
		{
			ASADCT = (reg & 0x200) >> 9;
			return (ASADCT);
		}
		bool fetch_ASFRST(uint16_t reg)
		{
			ASFRST = (reg & 0x400) >> 10;
			return (ASFRST);
		}
		uint8_t fetch_Reserved1(uint16_t reg)
		{
			Reserved1 = (reg & 0xF800) >> 11;
			return (Reserved1);
		}
	};
	...
	typedef struct aval_t //!< Angle Value register offset 0x02
	{
		...
		bool fetch_ANGVAL(uint16_t reg)
		{
			ANGVAL = (reg & 0x7FFF);
			return (ANGVAL);
		}
	...
	typedef struct aspd_t //!< Angle Speed register offset 0x03
	{
		...
		bool fetch_ANGSPD(uint16_t reg)
		{
			ANGSPD = (reg & 0x7FFF);
			return (ANGSPD);
		}
	...
	typedef struct mod1_t //!< MOD_1 Interface Mode1 register offset 0x06
	{
		...
		bool fetch_DSPUHOLD(uint16_t reg)
		{
			DSPUHOLD = (reg & 0x4) >> 2;
			return (DSPUHOLD);
		}
		...
		{
			Reserverd1 = (reg & 0x8) >> 3;
			return (Reserverd1);
		}
		bool fetch_CLKSEL(uint16_t reg)
		{
			CLKSEL = (reg & 0x10) >> 4;
			return (CLKSEL);
		}
		bool fetch_Reserverd2(uint16_t reg)
		{
			Reserverd2 = (reg & 0x3FE0) >> 5;
			return (Reserverd2);
		}
	...
	typedef struct sil_t //!< SIL register offset 0x07
	{
		...
		bool fetch_ADCTVEN(uint16_t reg)
		{
			ADCTVEN = (reg & 0x40) >> 6;
			return (ADCTVEN);
		}
		...
		{
			Reserverd1 = (reg & 0x380) >> 7;
			return (Reserverd1);
		}
		bool fetch_FUSEREL(uint16_t reg)
		{
			FUSEREL = (reg & 0x400) >> 10;
			return (FUSEREL);
		}
		uint8_t fetch_Reserverd2(uint16_t reg)
		{
			Reserverd2 = (reg & 0x3800) >> 11;
			return (Reserverd2);
		}
		bool fetch_FILTINV(uint16_t reg)
		{
			FILTINV = (reg & 0x4000) >> 14;
			return (FILTINV);
		}
		bool fetch_FILTPAR(uint16_t reg)
		{
			FILTPAR = (reg & 0x8000) >> 15;
			return (FILTPAR);
		}
	};
	...
	typedef struct mod2_t //!< MOD_2 Interface Mode2 register offset 0x08
	{
		...
		bool fetch_PREDICT(uint16_t reg)
		{
			PREDICT = (reg & 0x4) >> 2;
			return (PREDICT);
		}
		bool fetch_ANGDIR(uint16_t reg)
		{
			ANGDIR = (reg & 0x8) >> 3;
			return (ANGDIR);
		}
		...
		{
			ANGRANGE = (reg & 0x7FF0) >> 4;
			return (ANGRANGE);
		}
		bool fetch_Reserverd1(uint16_t reg)
		{
			Reserverd1 = (reg & 0x8000) >> 15;
			return (Reserverd1);
		}
	};
	...
	typedef struct mod3_t //!< MOD_3 Interface Mode3 register offset 0x09
	{
		...
		bool fetch_SSCOD(uint16_t reg)
		{
			SSCOD = (reg & 0x4) >> 2;
			return (SSCOD);
		}
		bool fetch_SPIKEF(uint16_t reg)
		{
			SPIKEF = (reg & 0x8) >> 3;
			return (SPIKEF);
		}
	...
	{                     //!< IFAB register offset 0x0d
		...
		bool fetch_IFABOD(uint16_t reg)
		{
			IFABOD = (reg & 0x4) >> 2;
			return (IFABOD);
		}
		bool fetch_FIRUDR(uint16_t reg)
		{
			FIRUDR = (reg & 0x8) >> 3;
			return (FIRUDR);
		}
	...
	{                       //!< D_Mag vector magnitude offset 0x14
		...
		uint8_t fetch_Reserverd1(uint16_t reg)
		{
			Reserverd1 = (reg & 0xFC00) >> 10;
			return (Reserverd1);
		}
	};

	typedef struct traw_t
	...
	{                        //!< register T250 offset 0x30
		...
		int16_t fetch_T250(uint16_t reg)
		{
			T250 = (reg & 0x1FFF);
			return (T250);
		}
